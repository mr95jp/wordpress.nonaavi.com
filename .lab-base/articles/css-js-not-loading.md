...
ブラウザの開発者ツール（Network タブ）で赤い行を探すのが基本ですが、
コマンドでまとめて確認できます。このラボの `bin/diagnose.sh` は
**ページ内の CSS / JS / 画像を実際に取得してステータスを数えます。**

```
$ bin/diagnose.sh https://example.com
  アセット 1 件: 404=1
  !! 404 [css] https://example.com/wp-content/themes/xxx/style.css?ver=0.1.0
       (404 でも本文は WordPress の 404 ページ = 20KB 前後の HTML が返る)
```

自分で確認するなら、これだけでも十分です。
