...
```sh
bin/logs.sh          # 4 種類をまとめて追う
bin/logs.sh clear    # 空にしてから追う
```

```
logs/apache/error.log        Apache
logs/nginx/error.log         nginx
logs/php/php_error.log       PHP(起動前のエラーもここ)
logs/php/lab.log             mu-plugin が書く検証用ログ
logs/mysql/error.log         MySQL
logs/mysql/slow.log          遅いクエリ
src/wp-content/debug.log     WordPress
```

`logs/php/lab.log` は mu-plugin が書いています。
**プラグインが Fatal で停止させられても、リカバリーモードで
全プラグインが読み込まれなくても記録が残る**ようにするためです。
記録層を通常のプラグインに置くと、いちばん知りたい瞬間に道連れで消えます。
