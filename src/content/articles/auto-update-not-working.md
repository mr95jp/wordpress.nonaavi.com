...
summary: |
  自動更新に任せているつもりでも、既定で自動になるのは本体のマイナー更新だけです。
  ・プラグインとテーマは、1 つずつ「自動更新を有効化」を押すまで更新されない
  ・プラグイン一覧に「自動更新」の列が無いなら、wp-config.php の設定で止められている
  ・更新の確認は 12 時間ごとの定期処理。アクセスが無いサイトでは走らない
  ・本体のメジャー更新は既定では自動にならない（人が判断する前提）
status: published
