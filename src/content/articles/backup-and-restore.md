...
summary: |
  戻すにはデータベースとファイルの両方が、同じ時点でそろっている必要があります。
  ・管理画面の「エクスポート」で書き出せるのは投稿とユーザーだけ。プラグイン・テーマ・設定・画像は含まれない
  ・データベースだけ戻すと、プラグインの有効化やサイト名は戻るが、ファイルは戻らない
  ・ファイルが無いプラグインの記録は自動で消えず、無視されるだけ（エラーも出ない）
  ・小規模なら書き出しは 1 秒未満で終わる。復元を一度試しておくことが唯一の確認方法
status: published
