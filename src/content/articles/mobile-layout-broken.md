...
summary: |
  まず、PC とスマホでサーバーが同じ HTML を返しているかを確かめます。
  ・同じ → 原因は CSS。viewport メタタグの有無と、画面幅からはみ出す要素（固定幅、画像、表、長い URL）を探す
  ・違う → 端末で出し分けている。キャッシュが PC 版をスマホに返していないかを見る
  ・特定のページだけ崩れ、キャッシュを消すと直り、ログイン中は正常 → キャッシュで確定。モバイル別キャッシュを有効にする
  ・スマホの検索結果から来たときだけ違う → AMP ページを見ている
status: published
...
キャッシュ系のプラグインを 2 つ入れている場合は、互いの設定が衝突していることもあります。
