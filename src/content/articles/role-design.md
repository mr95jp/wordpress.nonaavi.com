...
summary: |
  全員を管理者にする必要はありません。権限の数は管理者 61・編集者 34 で、差は 27 個です。
  ・記事の運用だけなら編集者で足りる。設定・プラグイン・テーマ・ユーザー管理は触れない
  ・管理者は保守の担当者だけにする。コードの書き換えも、他人を管理者に昇格させることもできる
  ・寄稿者は画像を追加できない。エラーではなく機能が消えるので、事前に伝えておく
  ・wp-config.php に DISALLOW_FILE_EDIT を入れると、管理画面からコードを編集できなくなる
status: published
