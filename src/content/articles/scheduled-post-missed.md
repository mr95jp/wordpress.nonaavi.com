...
summary: |
  WP-Cron は、サイトへのアクセスをきっかけにサイトが自分自身へ接続して動きます。
  ・アクセスの無い時間帯は、予約時刻を過ぎても公開されない
  ・アクセスしても公開されない → 自分自身への接続（ループバック）が失敗している。ツール > サイトヘルスにループバックの警告が出る
  ・WP-CLI の wp cron event run --due-now で公開されれば、処理自体は正常
  ・アクセスが少ないサイトは DISABLE_WP_CRON とサーバーの cron を両方設定する（片方だけだと定期処理が止まる）
status: published
...
あるいは投稿一覧の日付の欄に「**予約投稿の失敗**」と表示される。

![予約投稿の失敗](../screenshots/s/scheduled-post-missed.jpg)
...
なお、公開はされるが**ちょうど 9 時間ずれる**場合は、タイムゾーンの設定の問題です。
→ [時刻が 9 時間ずれる](emoji-and-timezone.md)
