...
summary: |
  どこが遅いかで原因が分かれます。
  ・全ページが遅い → wp_options の autoload の肥大（12MB で読み込み 15ms、メモリは 28MB から 52MB に）
  ・管理画面だけ遅い → 外部への通信。正常でも 1 回 0.8 秒、遮断されていると 5 秒待たされる
  ・特定のページだけ遅い → データベースのクエリ。slow.log の Rows_examined を見る
  キャッシュプラグインは測ってから入れます。ログイン中や検索結果のページは速くなりません。
status: published
...
|---|---|---|---|---|
| **42 KB**（検証環境の初期状態） | 127 | **0.7 ms** | 0.1 ms | 28 MB |
