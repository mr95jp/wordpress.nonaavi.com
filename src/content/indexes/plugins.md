...
実際に調べてみると、**プラグインは引き金であって、原因は別の層にある**ことが大半でした。
プラグインの設定を触る前に、下の表で本当の原因を確認してください。
プラグイン固有の挙動を実測できたものだけ、専用の記事があります。
...
取り方は [症状から引く](symptoms.md) の「最初に取る 4 つの値」にまとめてある。
